...
const calculateDataTime = courses => {
	const coursesTime = courses.map(({ length, name }) => {
		const time = length.split(':');
		const indexOf = name.indexOf('[');
		const module = indexOf !== -1 && +name[indexOf + 1];
		return { minutes: +time[0], seconds: +time[1], module };
	});

	return coursesTime;
};

const reduceData = (coursesTime, data) => {
	const courseData = coursesTime
...
const getRightTime = (time, { minutes, seconds }) => {
	const totalTime = (time + minutes) / 60;
	const totalMinutes = ((totalTime * 10) % 10) / 10;
	const totalHours = Math.round(totalTime - totalMinutes);
	return { totalHours, minutes: Math.floor(totalMinutes * 60), seconds, time };
};

const getModuleCourses = (data, module) => {
	if (!module) return data;
	return data.filter(el => el.module === module);
};

const getModuleCoursesTime = (courses, module) => {
	const moduleCourseTime = getModuleCourses(calculateDataTime(courses), module);
	const moduleCourseMinutes = reduceData(moduleCourseTime, 'minutes');
	const moduleCourseSeconds = reduceData(moduleCourseTime, 'seconds');
	const courseMinutes = convertSecondsToMinutes(moduleCourseSeconds);
	const courseHour = getRightTime(moduleCourseMinutes, courseMinutes);
	return courseHour;
};

const logTemplate = (
	{ totalHours, minutes, seconds, time },
	module,
	fullTime
) => {
	return `Module ${module} - statistics --- ${totalHours} hours  ${minutes} minutes ${seconds} seconds (% of course ${Math.round(
		(time / fullTime) * 100
	)})`;
};

const calculateEachCourseTime = courses => {
	const allCoursesTime = getModuleCoursesTime(courses, 0);
	const fullTime = allCoursesTime.time;
	const firstModuleCoursesTime = logTemplate(
		getModuleCoursesTime(courses, 1),
		1,
		fullTime
	);
	const secondModuleCoursesTime = logTemplate(
		getModuleCoursesTime(courses, 2),
		2,
		fullTime
	);
	const thirdModuleCoursesTime = logTemplate(
		getModuleCoursesTime(courses, 3),
		3,
		fullTime
	);
	...
	console.log(
		allCoursesTime,
		firstModuleCoursesTime,
		secondModuleCoursesTime,
		thirdModuleCoursesTime
	);
};

calculateEachCourseTime(lessons);
